...
import {AUTH_TOKEN} from '../../store/actions/ActionTypes';
...
class CartList extends React.PureComponent {
    ...
    componentDidMount() {
        if (AUTH_TOKEN) {
            axios.get(BASE_URL + '/cart/', { headers: { 'Authorization': `token ${AUTH_TOKEN}` } })
                .then(response => {
                    this.setState({
        ...
        if (AUTH_TOKEN) {
            ...
            if (existingCartIndex >= 0) {
                ...
                if (cart.add === true) {
                    ...
                    this.setState({ cart_total_amount: this.state.cart_total_amount + (cart.rate_per_week * cart.weeks) })
        ...
        if (AUTH_TOKEN) {
            ...
            if (existingCartIndex >= 0) {
                ...
                this.setState({
                    ...
                    cart_total_amount: this.state.cart_total_amount - (cart.count * cart.rate_per_week * cart.weeks)
                });
            }

            axios.delete(BASE_URL + `/remove/cart/${cart.id}/`, { headers: { 'Authorization': `token ${AUTH_TOKEN}` } })
                            ...
                            {(this.state.cart_count > 0 && this.props.isAuthenticated) ?
                                <span>
                                    ...
                                    <div id='cart-page-button' style={{ justifyContent: 'center', display: 'flex', flexWrap: 'wrap' }}>
                                        <a href='/products'><button id='continue'>Continue Shopping</button></a>
